...
BigInteger BigInteger::sqrt()   //Babylonian Method
{
    BigInteger r1 = *this / BigInteger(2);
    BigInteger r2 (2);

while (BigInteger((r2 - r1).getNumber()) > BigInteger(1))
    {
     r2 = (r1 + r2) / 2;
     r1 = *this / r2;
    }
...
//----------END OF FUNCTION DEFINITION OF BIGINTEGER LIBRARY ------------

/************** FUNCTION DEFINITION OF MODULO ARITHMETIC LIBRARY ************/



BigInteger ModuloArithmetic::Modulo(BigInteger &a,BigInteger &b)
{
    return a%b;
}

BigInteger ModuloArithmetic::ModuloAddition(BigInteger &a,BigInteger &b,BigInteger &c)
{
    return (a%c + b%c)%c;

}

BigInteger ModuloArithmetic::ModuloSubtraction(BigInteger &a,BigInteger &b,BigInteger &c)
{
    return (a%c - b%c)%c;

}

BigInteger ModuloArithmetic::ModuloMultiplication(BigInteger a,BigInteger b,BigInteger c)
{
    return (a%c * b%c)%c;

}
/*
BigInteger ModuloArithmetic::ModuloExponentiation(BigInteger a,BigInteger b,BigInteger c)
{
    if(b == BigInteger(1))return a%c;

    if(b%2 == BigInteger(0))return (ModuloExponentiation(a,b/2,c)*ModuloExponentiation(a,b/2,c))%c;
    else return (ModuloExponentiation(a,(b-1)/2,c)*ModuloExponentiation(a,(b-1)/2,c)*(a%c))%c;

}*/

BigInteger ModuloArithmetic::ModuloExponentiation(BigInteger a, BigInteger b,BigInteger c)
{
       BigInteger res("1"); BigInteger exp=b,term =a;
       for(int i=0;exp!=BigInteger(0);i++)
       {
...
BigInteger ModuloArithmetic::GCD(BigInteger a,BigInteger b)
{
    if(a<b)return a;
    return GCD(b,a%b);
}

BigInteger ModuloArithmetic::InverseModulo(BigInteger a,BigInteger n)
{
    BigInteger x, y;
    BigInteger g = ModuloArithmetic::gcdExtended(a, n, &x, &y);
    if (g != BigInteger(1))
    {
    ...
    else
    {
        ...
        BigInteger res = (x%n + n) % n;
        return res;
    }
}

BigInteger ModuloArithmetic::gcdExtended(BigInteger a, BigInteger b, BigInteger *x, BigInteger *y)
{
    // Base Case
    if (a == BigInteger(0))
    {
...
BigInteger ModuloArithmetic::DiscreteLogarithm(BigInteger a, BigInteger b, BigInteger c)
{
    BigInteger m=c.sqrt(),inv=InverseModulo(a,c);
    string baby="",giant="";
    for(BigInteger i(1);i<m;i++)
    {
        BigInteger modB=ModuloExponentiation(a,i,c);
        for(BigInteger j(1);j*m<=c;j++)
           {
              BigInteger modG= ModuloMultiplication(b,ModuloExponentiation(inv,j*m,c),c);
              if(modB==modG) return i+m*j;
           }
    }
    return BigInteger(-1);
}

BigInteger ModuloArithmetic::GenerateEncryptedMessage(BigInteger message,BigInteger PublicKeyExponend,BigInteger PublicKeyBase)
{
    return ModuloExponentiation(message,PublicKeyExponend,PublicKeyBase);
}

BigInteger ModuloArithmetic::GenerateDecryptedMesssage(BigInteger EncryptedMessage,BigInteger d,BigInteger n)
{
    return ModuloExponentiation(EncryptedMessage,d,n);
}
